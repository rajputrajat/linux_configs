...
const config_1 = __webpack_require__(63);
const ctx_1 = __webpack_require__(38);
const downloader_1 = __webpack_require__(46);
const inlay_hints_1 = __webpack_require__(64);
...
Object.defineProperty(exports, "__esModule", { value: true });
const coc_nvim_1 = __webpack_require__(1);
const ra = __importStar(__webpack_require__(4));
const sourceChange = __importStar(__webpack_require__(35));
...
exports.applySourceChange = applySourceChange;
function selectAndApplySourceChange() {
    return async (changes) => {
        if ((changes === null || changes === void 0 ? void 0 : changes.length) === 1) {
            await sourceChange.applySourceChange(changes[0]);
        }
        else if ((changes === null || changes === void 0 ? void 0 : changes.length) > 0) {
            const pick = await coc_nvim_1.workspace.showQuickpick(changes.map((c) => c.label));
            if (pick) {
...
exports.selectAndApplySourceChange = selectAndApplySourceChange;
...
function request(method) {
    return new vscode_languageserver_protocol_1.RequestType(`rust-analyzer/${method}`);
}
function notification(method) {
    return new vscode_languageserver_protocol_1.NotificationType(method);
}
exports.analyzerStatus = request('analyzerStatus');
exports.collectGarbage = request('collectGarbage');
exports.syntaxTree = request('syntaxTree');
exports.expandMacro = request('expandMacro');
exports.findMatchingBrace = request('findMatchingBrace');
exports.decorationsRequest = request('decorationsRequest');
exports.parentModule = request('parentModule');
exports.joinLines = request('joinLines');
exports.onEnter = request('onEnter');
exports.runnables = request('runnables');
exports.inlayHints = request('inlayHints');
exports.ssr = request('ssr');
exports.publishDecorations = notification('publishDecorations');
...
async function applySourceChange(change) {
    ...
    if (change.workspaceEdit.documentChanges) {
        for (const docChange of change.workspaceEdit.documentChanges) {
            if (vscode_languageserver_protocol_1.CreateFile.is(docChange)) {
                created = docChange.uri;
            }
            else if (vscode_languageserver_protocol_1.RenameFile.is(docChange)) {
    ...
    if (toOpen) {
        await coc_nvim_1.workspace.jumpTo(coc_nvim_1.Uri.parse(toOpen).toString());
    }
    else if (toReveal) {
        const uri = toReveal.textDocument.uri;
        const position = toReveal.position;
        coc_nvim_1.workspace.jumpTo(uri, position);
    }
}
exports.applySourceChange = applySourceChange;
...
Object.defineProperty(exports, "__esModule", { value: true });
const coc_nvim_1 = __webpack_require__(1);
const executable_1 = __importDefault(__webpack_require__(39));
const fs_1 = __webpack_require__(2);
const os_1 = __webpack_require__(16);
const path_1 = __webpack_require__(15);
const vscode_languageserver_protocol_1 = __webpack_require__(5);
const which_1 = __importDefault(__webpack_require__(41));
const client_1 = __webpack_require__(45);
const downloader_1 = __webpack_require__(46);
const status_display_1 = __webpack_require__(53);
...
const pify = __webpack_require__(40);
...
const isexe = __webpack_require__(42)
...
const coc_nvim_1 = __webpack_require__(1);
const fs_1 = __webpack_require__(2);
const node_fetch_1 = __importDefault(__webpack_require__(47));
...
async function downloadServer(context, updatesChannel) {
    ...
    return new Promise((resolve, reject) => {
        node_fetch_1.default(latest.url)
            .then((resp) => {
            let cur = 0;
            ...
            resp.body
                ...
                .pipe(fs_1.createWriteStream(_path, { mode: 0o755 }));
        })
            .catch((e) => {
            statusItem.hide();
            reject(e);
...
/* harmony import */ var http__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(49);
/* harmony import */ var url__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(50);
/* harmony import */ var https__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(51);
/* harmony import */ var zlib__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(52);
...
const ctx_1 = __webpack_require__(38);
const ra = __importStar(__webpack_require__(4));
const source_change_1 = __webpack_require__(35);
function joinLines(ctx) {
    return async () => {
        ...
        await source_change_1.applySourceChange(change);
    };
}
...
function onEnter(ctx) {
    return async () => {
        ...
        let lastChar;
        let lastTS = 0;
        coc_nvim_1.events.on('InsertCharPre', (char) => {
            lastChar = char;
            lastTS = Date.now();
        });
...
Config.requiresReloadOpts = ['serverPath', 'cargo', 'procMacro', 'files', 'updates'].map((opt) => `${Config.rootSection}.${opt}`);
...
class HintsUpdater {
    ...
    syncCacheAndRenderHints() {
        ...
        this.sourceFiles.forEach((file, uri) => this.fetchHints(file).then((hints) => {
            ...
            for (const doc of coc_nvim_1.workspace.documents) {
                if (doc.uri === uri) {
    ...
    async renderDecorations(decorations) {
        const doc = await coc_nvim_1.workspace.document;
        if (!doc)
            return;
        doc.buffer.clearNamespace(this.chainingHintNS);
